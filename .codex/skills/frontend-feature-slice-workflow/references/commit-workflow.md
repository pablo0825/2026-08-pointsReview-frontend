...
5. 只 stage 本 batch 的明確檔案。
6. 檢查 staged file list、staged diff 與排除項目。
7. 使用核准 message 建立 commit。
8. 記錄 Commit ID、message、檔案、排除項目與驗證結果。
9. continuous sequence 直接進入下一 batch；最後一個 implementation batch 後直接執行完整 AI Verification。
...
| Verification | `docs(<ID>): record <feature> verification` | Verification、Plan 結果與 blueprint 狀態；回報後停止 |
...
只改變 batch 分組、順序、Files、Required Verification 或 message 時，更新 Plan 並將 Commit Plan Approval 設為 `pending`，等待重新核准後才執行受影響 batch。
...
Git history 是 Commit ID 與詳細變更歷史的權威來源；文件不記錄自身尚未產生的 hash，也不重建完整 commit history。
