...
test('submits a normalized single-participant application after an explicit estimate', async ({
  ...
  await expect(page.getByRole('heading', { name: '計畫與薪資試算' })).toBeVisible()
  await expect(page.getByText('每月 1～50,000 元，最多 12 個不同月份。')).toHaveCount(0)
  ...
  await expect(page.getByText('王小明｜4A0X0001｜大三 甲班')).toBeVisible()
  await expect(page.getByText('預估點數：4.00 點')).toBeVisible()
  ...
  expect(submittedBody).toContain('"grade":3')
  expect(submittedBody).toContain('"classNumber":1')
