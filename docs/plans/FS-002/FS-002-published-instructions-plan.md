...
重用既有公開頁骨架、Router 與 Query Provider，建立 `/rules` 的年度公開申請辦法資料存取、安全 Markdown 文章、目錄、頁內錨點與完整頁面狀態，並以元件、整合及 Chromium browser tests 驗證安全性、年度切換、鍵盤操作與 360px 版面。
...
- 建立 feature-local 的公開申請辦法 wire schema、mapper、query key、query function 與目前臺灣學年度 helper。
...
- 以正式頁面取代 `/rules` placeholder，涵蓋學年度切換、loading、empty、error、success、retry、鍵盤、輔助科技與 360px 狀態。
...
- `src/app/providers/app-providers.tsx`：重用既有 Query Client；本 Slice 以 feature query key 隔離各學年度快取。
...
- `GET /public/application-instructions`：依所選學年度取得已發布且可見的申請辦法。
- `src/app/providers/app-providers.tsx`：頁面使用 TanStack Query 管理每學年度的 server state 與取消訊號。
...
- `/rules` 尚無正式頁面、學年度選擇器或任何資料查詢。
- 尚無公開申請辦法的 wire contract、response validation、mapper、query 或 API client。
- 尚無 Markdown rendering、allowlist sanitization、heading anchors 或目錄。
- 尚無 loading、empty、error、retry、快速年度切換或安全內容測試。
...
- `FS-002` 沒有 blueprint dependency，但資料層 batch 必須先取得正式 API wire contract。

### Unknowns

- `GET /public/application-instructions` 的 query parameter、response fields、Markdown field、available years source，以及 empty／hidden／unpublished 表示方式尚未定義。
- 公開端點是否以 server-side filtering 保證只回傳已發布且可見內容，仍待後端契約確認。
...
- `src/features/rules/api/published-instructions.schema.ts`：正式 wire response Zod schema。
- `src/features/rules/api/published-instructions.mapper.ts`：wire data 至頁面 view model 的明確轉換。
- `src/features/rules/api/published-instructions.query.ts`：年度 query key 與 abortable query function。
- `src/features/rules/api/published-instructions.test.ts`：schema、mapper、成功、empty、error 與 contract failure 測試。
...
- `src/features/rules/published-instructions-page.tsx`：年度選擇與所有 query states 的頁面組合。
- `src/features/rules/published-instructions-page.test.tsx`：MSW 驗證年度切換、loading、empty、error、retry、success 與 stale result 防護。
- `src/test/server.ts`：共用 MSW Node server。
- `src/test/handlers/public-application-instructions.ts`：預設公開辦法 handler 與測試覆寫 helpers。
- `src/test/fixtures/published-instructions.ts`：不含真實個資且通過正式 schema 的目前／歷史年度 fixtures。
- `e2e/published-instructions.spec.ts`：desktop／360px、鍵盤、年度、目錄、連結、狀態與 overflow browser test。
...
- `src/features/rules/api/published-instructions.test.ts`：wire schema、mapper、query input/output 與錯誤。
...
- `src/features/rules/published-instructions-page.test.tsx`：MSW query states、年度切換、retry 與 stale result。
...
2. 在正式 wire contract 已確認的前提下，建立最小共用 GET JSON client，以及 feature-local schema、mapper、年度 query、目前臺灣學年度 helper、MSW server／fixture／handler 與對應測試。
...
4. 建立 `PublishedInstructionsPage`，以所選學年度驅動 query key，組合 selector、loading、empty、error／retry 與 success article，避免前一年度結果誤標。
5. 將正式頁面接入 `/rules`，更新 Router 與既有 application-entry tests，保留 Public Layout、根 redirect 與四個申請目的 routes。
6. 建立 Chromium browser flow，驗證未登入公開存取、年度切換、長篇目錄、鍵盤、44px targets、desktop／360px 與無水平溢位。
...
|---|---|---|
| 公開申請辦法正式 wire contract 未定義 | 無法建立正確 schema、mapper、MSW fixture 或真實整合 | 在 I2 前由使用者／後端確認 query、response、available years、empty 表示與 server-side visibility guarantee；若需求文件需變更，先提出並核准文件修訂 |
...
| 年度切換產生並行 request | 舊內容可能誤顯示為新年度 | Query key 包含正規化年度、傳遞 AbortSignal，且 renderer 只接受目前 query 對應 view model |
...
- [ ] 建立正式公開申請辦法 schema、mapper、query 與臺灣學年度 helper。
- [ ] 建立嚴格 MSW lifecycle、fixtures 與 handlers。
- [ ] 建立安全 Markdown article、唯一 heading anchors、目錄與安全連結。
- [ ] 建立 `/rules` 年度選擇與 loading、empty、error、success、retry 狀態。
...
- [ ] 提供可由真實公開端點取得的目前學年度、至少一個歷史學年度與一個無公開內容學年度測試資料。
...
- [ ] 在 360px 寬度切換目前與歷史學年度，確認控制項、目錄與文章易於閱讀和操作。
...
- [ ] `not-applicable`；目前沒有已核准且需納入本 Plan 的 `docs/project/` 需求文件變更。
