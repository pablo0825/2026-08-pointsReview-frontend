...
| Revised Target Behavior | 修訂後 Spec 與自動化測試逐項比對 | passed | Component request-count assertions 與 Chromium flow | 未發現修訂後 Spec 落差。 |
...
| 每個申請類型只查詢一次，年度切換不新增 request | passed | Component 與 Chromium 均斷言選擇競賽後為 1 個 request，切換 114 年後仍為 1 個。 |
...
| F1 | 每個申請類型改為單次查詢並以前端篩選年度 | targeted Vitest、typecheck、lint、test、build、Chromium Playwright | passed | `fix(rules): avoid duplicate instructions requests` |
| Reverification | 保存修訂後完整 AI Verification 與狀態 | 完整證據、文件一致性、`git diff --check` | passed | `docs(FS-002): update published instructions verification` |

## Human Integration

### Required Work

1. 在真實環境確認頁面對每個 `applicationType` 只呼叫一次 `GET /public/application-instructions`，且 request 省略 `academicYear`。
...
| 2 | 逐一選擇四種申請類型並觀察 Network。 | 預設目前臺灣學年度，內容不會混用其他類型；每次類型選擇只產生一個省略 `academicYear` 的 request。 |
| 3 | 切換目前與歷史學年度並觀察 Network。 | 每次顯示對應年度且不新增 request；`data: []` 顯示無內容而非錯誤。 |
