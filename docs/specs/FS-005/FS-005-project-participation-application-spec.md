...
1. 頁面固定使用「計畫內容與薪資試算」、「申請人資料」、「指導老師」、「附件」、「確認送出」五步；已完成且位於目前步驟之前的進度項目可返回，目前與後續步驟不可跳轉，返回後須逐步重新驗證。
...
5. `salaryAmount` 必須是 1～50,000 的 JSON 整數；不接受零、負數、小數或超出上限的值。
...
13. 唯一參與者沿用共用姓名、學號、年級、班級、Email 與電話規則；學號 trim 後轉大寫，`applicant.name` 取 trim 後的 `studentName`。
...
17. 確認頁依序顯示「計畫與薪資」、「申請人資料」、「指導老師」、「附件」四個唯讀摘要，不顯示 `applicationType`、`academicYear`、`isApplicant`、`clientFileKey`、`Idempotency-Key` 或區塊內修改按鈕。
...
21. 正式 `422` 依 `fields[].path` 返回「計畫內容與薪資試算」、「申請人資料」或附件區塊並就地顯示；正式點數低於 0.50 時將 `typeDetails.salaryItems` 錯誤顯示於薪資試算區塊並要求重新試算。
...
- [ ] TypeScript、ESLint、Vitest、Production Build 與相關 Chromium Playwright 全部通過。
- [ ] 自動測試五步順序、進度導覽、逐步驗證、離開警告及 360px／鍵盤可用性。
- [ ] 自動測試計畫欄位 trim、長度、工作內容 1,000 字及欄位內錯誤呈現。
- [ ] 自動測試薪資 1～12 筆、月份轉換／重複／未來、1～50,000 整數及增刪邊界。
- [ ] 自動測試只有明確操作才呼叫試算、`isEligible` 兩種成功狀態、422 路徑、Network／5xx、手動重試與薪資異動失效。
- [ ] 自動測試點數唯讀、前端不自行換算、單一申請人、學號大寫及動態學年度隱藏但存在 payload。
- [ ] 自動測試老師失敗／空資料／重新載入、必要 `salary_proof`、附件分類／技術限制及確認頁四區塊。
- [ ] 自動測試 multipart payload、Idempotency Key、重複點擊、結果不確定快照重試、409、429、未知錯誤、附件錯誤與 201 成功頁。
- [ ] 回歸驗證 `/apply` 與 `/apply/competition` 的既有行為及測試不受影響。

## Human Acceptance

- [ ] 以真實後端驗證合格多月薪資只在按下按鈕後試算、點數唯讀，修改薪資後須重新試算。
- [ ] 以真實後端驗證 `isEligible: false` 顯示未達門檻並禁止繼續，以及未來／重複月份錯誤定位。
- [ ] 完成單一申請人、老師與薪資證明附件的真實 multipart 送件，確認後端只建立一筆資料且內容正確。
- [ ] 確認確認頁四區塊、進度返回修改、動態學年度不顯示、欄位錯誤與 360px 版面可理解且可操作。
- [ ] 確認成功頁申請編號、等待老師簽核、送件時間與 Email 提醒正確，且不顯示老師預計簽核日期。
