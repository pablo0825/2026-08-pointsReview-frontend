...
- [ ] 自動測試確認頁不顯示分配方式或區塊「修改」按鈕，申請人與聯絡資料置頂、其他參與者維持原順序，且 payload 參與者順序不變。
- [ ] 自動測試進度列的目前、可返回與 disabled 狀態；從確認頁返回後不能跳過後續步驟，必須依序重新驗證，且資料保留。
...
- [ ] 確認初始未預選申請人且聯絡資料隱藏；未選申請人時錯誤與焦點正確，選定後聯絡資料顯示在該參與者卡片，改選時清除聯絡資料且不可直接取消目前申請人。
...
- [ ] 確認競賽表單與確認頁不顯示學年度，但正式 payload 的每位參與者仍包含系統學年度。
- [ ] 確認 `shared_total` 每位參與者從 0.00 開始，摘要位於清單上方，0.50 加減按鈕與手動輸入可用，且不能用按鈕分配成負數或超過總點數。
- [ ] 確認確認頁不顯示分配方式或區塊「修改」按鈕，申請人在參與者摘要置頂且 Email、電話緊接於其下；進度列只能返回前面已完成步驟，返回後必須依序重新驗證。
