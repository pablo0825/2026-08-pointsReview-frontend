...
- 系統依 Asia/Taipei 計算但不顯示的共用學年度、1–10 位參與者、申請人身分、Email 與電話。
...
1. 表單固定為「競賽內容」、「參與者資料」、「指導老師」、「附件」、「確認送出」五步；第一步先確立競賽規則，第二步才依規則填寫參與者與分配點數。下一步前驗證目前步驟，可定位錯誤時聚焦第一個錯誤欄位，上一步與確認頁返回修改不得清除資料。進度列只允許點擊目前步驟之前已完成的階段；目前與後續階段不可點擊並以灰色 disabled 狀態顯示。返回前面階段後不可直接跳回後續階段，必須使用「下一步」重新逐步驗證。
2. 學年度依 Asia/Taipei 當日及 8 月 1 日分界自動計算目前民國學年度，在競賽表單與確認頁皆不顯示，也不提供歷史學年度選擇；送件時仍複製到每位參與者的 `academicYear`。
...
15. `shared_total` 顯示「團隊總點數分配」。不論一人或多人，每位參與者初始為 0.00，不自動平均或預先分配；完成分配時每人不得低於 0.50、必須是 0.50 的倍數，且總和必須精確等於規則 `points` 才能繼續或送件。
16. `shared_total` 在參與者清單上方顯示團隊總點數、已分配點數及剩餘點數，第一版不使用 sticky；每位參與者提供明確的「−」與「＋」按鈕，每次調整 0.50，並保留手動輸入。隱藏瀏覽器原生數字上下箭頭；0.00 時停用「−」，剩餘點數不足 0.50 時停用會超額的「＋」，手動輸入仍須通過完整驗證。
17. `shared_total` 的實際人數上限為競賽固定上限 10 與 `FLOOR(points / 0.50)` 的較小值；新增參與者保留既有分配並將新參與者設為 0.00，移除參與者後其點數回到未分配額。
18. 切換競賽等級或獎項時保留參與者名單但清除舊規則點數；新規則為 `per_person` 時改用新固定點數，新規則為 `shared_total` 時將所有參與者重設為 0.00 後重新分配。
19. 點數加總、倍數、最低值與相等比較不得直接使用 JavaScript 浮點數，畫面與 wire payload 使用兩位小數字串。
20. 首次進入指導老師步驟時只呼叫一次不帶 Query Parameter 的 `GET /public/advisors`，返回其他步驟後沿用記憶體資料；搜尋只比對姓名、中文職稱與系所。
21. 老師顯示「姓名｜職稱｜系所」，`titleCode` 1–7 映射既定中文職稱，異常值顯示「未知職稱」；`isDirector` 不顯示且不參與搜尋，送件只傳選取老師的 `id` 為 `advisorId`。
22. 老師讀取失敗時顯示「暫時無法載入指導老師名單」，空陣列時顯示「目前沒有可選擇的指導老師」；兩者提供「重新載入」並禁止繼續。重載後原選擇不在清單時清除選擇。
23. 附件只接受 PDF、JPEG、PNG，每個檔案不得超過 5,242,880 bytes，每份申請最多 10 個；前端先依副檔名與 `File.type` 提示，正式判斷仍由後端執行。
24. 每個附件使用唯一、1–80 字元且只含英文字母、數字、底線或連字號的 `clientFileKey`，metadata 與 `attachments[{clientFileKey}]` 檔案必須一一對應；刪除附件一併移除 metadata、檔案及 Key。
25. 競賽附件分類只接受競賽辦法、海報、官網截圖、公文、參賽證明、入圍或獎狀、其他；至少須有一份 `participation_proof` 或 `finalist_or_award_certificate`。
26. 選擇附件分類 `other` 時其他名稱必填、trim 後 1–100 字元；切換離開時清除並傳 `null`。說明選填且最長 500 字元，trim 後空白傳 `null`。
27. 相同檔名、大小與 `lastModified` 的檔案再次加入時先提示可能重複，使用者可取消或仍加入；每份仍占一個附件名額。
28. JPEG 與 PNG 可縮圖或檢視，PDF 可在新分頁開啟；本機預覽 URL 在關閉、移除或表單卸載時釋放。替換檔案保留分類與說明但產生新 Key，替換檔案無效時保留原附件。
29. 確認頁以四個唯讀摘要區塊顯示申請人與參與者、競賽資料、指導老師、附件；不顯示學年度或由後端規則決定的分配方式。參與者摘要只在顯示層將申請人排在第一位，以「姓名｜學號｜個別點數（申請人）」顯示，並在其下方顯示 Email 與電話；其他參與者依原表單順序顯示，不改變表單或 payload 的 `participants` 順序。各區不提供「修改」按鈕，改由頁首進度列返回對應的已完成步驟；不新增確認勾選框。
30. 送件按鈕上方提醒資料與附件將交由所選指導老師簽核；按鈕文字為「確認送出申請」，處理中顯示「送件中…」並鎖定表單與按鈕。
31. 只載入規則或老師不算表單修改；使用者輸入、增減參與者、選老師或加入附件後才啟用未送出資料警告。步驟間切換不警告，站內離開、瀏覽器上一頁、重新整理或關閉分頁須警告；成功送件後解除，結果不確定時保留。
32. 正式送件使用 `POST /public/applications` multipart，`payload` 是 JSON 字串，檔案欄位為 `attachments[{clientFileKey}]`；request 使用 `credentials: omit`，不取得或攜帶 Session、Cookie 或 `X-CSRF-Token`。
33. 每次新的邏輯送件產生 UUID v4 `Idempotency-Key`。送出前建立不可變 payload 與附件快照；明確 `201` 後清除 Key 與快照，使用者修改 payload 或附件後也廢棄舊 Key 與快照。
34. Network Error、timeout、連線中斷、未收到明確 Response 或任何 5xx 都顯示「無法確認是否送件成功」，不宣告成功或失敗且不自動重試；「重新確認送件」必須沿用相同 Key、payload、附件內容、檔名、MIME type 與 `clientFileKey`，「返回修改資料」則廢棄快照並於下次使用新 Key。
35. `409 idempotency_key_conflict` 廢棄原 Key、返回表單確認並在下次送件產生新 Key；`429 rate_limited` 保留資料且不自動重試，可讀到 `Retry-After` 時顯示等待時間並停用送件，否則顯示通用稍後再試。
36. 前端驗證與 `422 validation_failed` 都依欄位 path 定位步驟及控制項；可定位的錯誤在控制項正下方顯示文字訊息，控制項使用紅色邊框、`aria-invalid` 與錯誤描述關聯，同一錯誤不在頁面頂端重複顯示，並聚焦第一個錯誤。跨欄位、點數總和、參與者集合或必要附件錯誤顯示於最接近的相關區塊；沒有 `fields`、無法定位的業務錯誤、Network Error、5xx、Rate Limit 與其他系統錯誤使用頁面內提示區塊，不使用瀏覽器原生 `alert()`。`fields[].path` 使用點號與數字索引且不依賴錯誤順序；`Required` 顯示「此欄位為必填」，未知欄位訊息使用通用中文 fallback，沒有 `fields` 時顯示頂層 `message`。
37. 已知 `400 file_too_large`、`too_many_files` 與 `file_type_not_allowed` 顯示對應附件訊息；未知 4xx 顯示可用的非空白後端訊息或通用中文訊息，保留表單、返回確認頁、廢棄原 Key，且不自動重試。
38. 後端回報規則已失效時，保留其他資料、返回「競賽內容」並要求手動重載；組合仍存在時依新規則重設點數，不存在時清除等級與獎項選擇。個別點數或參與者總和錯誤返回「參與者資料」。
39. 成功 Response 必須是 `201` 且包含 `publicId`、固定 `pending_advisor` 與 UTC ISO 8601 `submittedAt`；成功頁以 Asia/Taipei 顯示送件時間及 Email 通知提醒，不顯示老師簽核期限。
...
- [x] 自動測試競賽表單與確認頁不顯示學年度，但 payload 仍為所有參與者帶入系統計算學年度。
...
- [x] 自動測試 `shared_total` 全員 0.00 初始化、新增參與者為 0.00、摘要位於清單上方、自訂 0.50 加減按鈕、手動輸入、按鈕邊界及總和驗證。
- [x] 自動測試既有 `per_person`、單人與多人 `shared_total`、增減參與者、切換規則、最低值、0.50 倍數與整數點數加總。
...
- [x] 自動測試確認頁不顯示分配方式或區塊「修改」按鈕，申請人與聯絡資料置頂、其他參與者維持原順序，且 payload 參與者順序不變。
- [x] 自動測試進度列的目前、可返回與 disabled 狀態；從確認頁返回後不能跳過後續步驟，必須依序重新驗證，且資料保留。
...
- [x] 自動測試前端與 422 欄位錯誤皆顯示於對應控制項下方，控制項具有紅色邊框、`aria-invalid` 與錯誤描述關聯，修正後清除該欄位錯誤。
- [x] 自動測試跨欄位／集合錯誤留在相關區塊，無法定位的業務、網路與系統錯誤使用頁面內提示，且不呼叫瀏覽器原生 `alert()`。

## Human Acceptance

- [x] 在真實後端完成一筆 `per_person` 競賽申請，確認規則、老師、附件、確認頁與成功頁資料正確。
- [x] 在真實後端完成一筆多人 `shared_total` 競賽申請，確認分配限制、總和及正式送件正確。
- [x] 確認初始未預選申請人且聯絡資料隱藏；未選申請人時錯誤與焦點正確，選定後聯絡資料顯示在該參與者卡片，改選時清除聯絡資料且不可直接取消目前申請人。
- [x] 確認成功頁顯示申請編號、等待指導老師簽核狀態、台北送件時間及 Email 通知提醒，且不顯示老師簽核期限。
- [x] 確認真實規則／老師 failure、重載與 Network Error 結果不確定重新確認流程可理解且不造成重複案件。
- [ ] 真實規則／老師 empty、Rate Limit、5xx 與規則失效：`not-run`；缺少安全測試條件，由使用者接受為非阻擋項目。
- [x] 在桌面與 360px 手機瀏覽器以鍵盤／觸控完成主要流程，確認焦點、錯誤提示、離開警告與附件操作可用。
- [x] 確認錯誤輸入框具有紅色邊框且訊息緊鄰欄位下方，區塊與系統錯誤出現在正確層級，不需在頁面頂端尋找欄位錯誤。
- [x] 確認競賽表單與確認頁不顯示學年度，但正式 payload 的每位參與者仍包含系統學年度。
- [x] 確認 `shared_total` 每位參與者從 0.00 開始，摘要位於清單上方，0.50 加減按鈕與手動輸入可用，且不能用按鈕分配成負數或超過總點數。
- [x] 確認確認頁不顯示分配方式或區塊「修改」按鈕，申請人在參與者摘要置頂且 Email、電話緊接於其下；進度列只能返回前面已完成步驟，返回後必須依序重新驗證。
