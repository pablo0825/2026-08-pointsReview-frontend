...
- 訪客可指定申請類型；選定類型後，頁面預設顯示目前臺灣學年度的公開申請辦法。
...
2. 公開辦法查詢必須提供 `applicationType`；頁面不得把一種申請類型的內容標示為另一種類型。
3. `academicYear` 是選填的查詢條件；省略時取得所選申請類型的所有公開學年度，提供時只取得指定學年度。
4. 選定申請類型後，頁面預設顯示目前臺灣學年度。
5. 學年度選擇器只顯示所選申請類型可供公開閱讀的學年度，並允許切換目前與歷史內容。
6. 頁面只呈現後端提供的已發布且可見內容，不得顯示草稿、未發布或隱藏內容。
7. 同一申請類型與學年度可包含一個或多個 sections；每個 section 必須保留其標題與 Markdown 內容。
8. Markdown 標題必須具有同頁唯一的錨點；目錄項目必須對應並可前往相關標題。
9. Markdown 或內嵌 HTML 中的腳本、事件處理屬性、危險 URL 與其他可執行內容不得進入頁面；文章連結必須具備安全行為。
10. 查詢期間顯示載入狀態；查無公開內容時顯示所選申請類型與學年度的無內容狀態；查詢失敗時顯示安全、可理解的錯誤訊息與重試操作。
11. 切換申請類型或學年度後，不得把前一次查詢內容誤標為目前選擇的新結果。
12. 非同步狀態須能被輔助科技辨識；申請類型控制項、學年度選擇器、重試按鈕與目錄連結須可使用鍵盤操作，主要互動目標至少為 44×44px。
13. 內容、目錄與控制項在 360px 寬度下不得遮蔽主要資訊或造成非必要的水平捲動。
14. 錯誤畫面不得向訪客暴露內部端點細節、堆疊、token 或其他敏感資訊。
15. 本切片不包含管理員建立、編輯、預覽、發布或調整可見性的功能。
