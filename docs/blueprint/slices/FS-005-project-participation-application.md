...
## AI Responsibility
- 完成垂直功能、安全整數薪資與點數計算、API／錯誤、測試與文件；未登入正式送件不得索取 Session 或 CSRF Token，且須以必填 Idempotency Key 與不可變快照防止同一次操作重複建立案件。

## Human Integration
- 後端需完成 B10、點數預估與正式送件契約並提供測試資料。

## AI Acceptance
- 自動驗證月份、整數金額、先加總再換點、未滿千元阻擋、附件、Idempotent retry 與送件狀態。
