...
## AI Responsibility
- 完成垂直功能、學號大寫正規化、API／錯誤處理、測試與文件；未登入正式送件不得索取 Session 或 CSRF Token，須以必填 Idempotency Key 與不可變快照防止同一次操作重複建立案件，所有 5xx 皆依結果不確定流程以相同快照手動重新確認，且不自行決定證照編號正規化規則。

## Human Integration
- 確認證照編號正規化與防重策略，完成 B10–B11、B13 並提供上限／重複案例資料。

## AI Acceptance
- 自動驗證學號正規化、日期、固定點數、上限提示、必要附件、步驟、Idempotent retry 與已知重複錯誤呈現。
