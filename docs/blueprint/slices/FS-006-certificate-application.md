...
## AI Responsibility
- 完成垂直功能、API／錯誤處理、測試與文件；未登入正式送件不得索取 Session 或 CSRF Token，須以必填 Idempotency Key 與不可變快照防止同一次操作重複建立案件，且不自行決定證照編號正規化規則。

## Human Integration
- 確認證照編號正規化與防重策略，完成 B10–B11 並提供上限／重複案例資料。

## AI Acceptance
- 自動驗證日期、固定點數、上限提示、必要附件、步驟、Idempotent retry 與已知重複錯誤呈現。
